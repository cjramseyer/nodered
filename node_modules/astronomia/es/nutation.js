...
import sexa from './sexagesimal'; // Nutation: Chapter 22, Nutation and the Obliquity of the Ecliptic.
...
export function nutation(jde) {
  var T = base.J2000Century(jde); // Mean elongation of the Moon from the sun

  var D = base.horner(T, 297.85036, 445267.11148, -0.0019142, 1.0 / 189474) * Math.PI / 180; // Mean anomaly of the Sun (Earth)

  var M = base.horner(T, 357.52772, 35999.050340, -0.0001603, -1.0 / 300000) * Math.PI / 180; // Mean anomaly of the Moon

  var N = base.horner(T, 134.96298, 477198.867398, 0.0086972, 1.0 / 56250) * Math.PI / 180; // Moon's argument of latitude

  var F = base.horner(T, 93.27191, 483202.017538, -0.0036825, 1.0 / 327270) * Math.PI / 180; // Longitude of the ascending node of the Moon's mean orbit on the ecliptic, measured from mean equinox of date
...
export function approxNutation(jde) {
  ...
  var _base$sincos3 = base.sincos(2 * L),
      s2L = _base$sincos3[0],
      c2L = _base$sincos3[1];
  ...
  var _base$sincos5 = base.sincos(2 * Ω),
      s2Ω = _base$sincos5[0],
      c2Ω = _base$sincos5[1];
