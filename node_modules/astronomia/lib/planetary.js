...
function mean(y, a) {
  ...
  var J = a.A + k * a.B;
  var M = _base["default"].pmod(a.M0 + k * a.M1, 360) * Math.PI / 180;

  var T = _base["default"].J2000Century(J);
...
function sum(T, M, c) {
  ...
  var j = _base["default"].horner(T, c[0]);
  ...
  for (var i = 1; i < c.length; i++) {
    ...
    var _base$sincos = _base["default"].sincos(mm),
        _base$sincos2 = _slicedToArray(_base$sincos, 2),
        smm = _base$sincos2[0],
        cmm = _base$sincos2[1];

    j += smm * _base["default"].horner(T, c[i]);
    i++;
    j += cmm * _base["default"].horner(T, c[i]);
  }
...
function sumA(T, M, c, aa) {
  ...
  for (var k = 0; k < aa.length; k++) {
    var _base$sincos3 = _base["default"].sincos((aa[k].c + aa[k].f * T) * Math.PI / 180),
        _base$sincos4 = _slicedToArray(_base$sincos3, 2),
        _saa = _base$sincos4[0],
        caa = _base$sincos4[1];

    j += _saa * _base["default"].horner(T, c[i]);
    i++;
    j += caa * _base["default"].horner(T, c[i]);
    i++;
  }
