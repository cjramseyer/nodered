...
function heliocentric(jde) {
  ...
  for (var i in t37) {
    ...
    var _base$sincos = _base["default"].sincos((t.i * J + t.j * S + t.k * P) * Math.PI / 180),
...
function astrometric(jde, earth) {
  var sε = _base["default"].SOblJ2000;
  var cε = _base["default"].COblJ2000;

  var f = function f(jde) {
    var _heliocentric = heliocentric(jde),
        lon = _heliocentric.lon,
        lat = _heliocentric.lat,
        range = _heliocentric.range;
    ...
    var _base$sincos5 = _base["default"].sincos(lat),
        _base$sincos6 = _slicedToArray(_base$sincos5, 2),
        sb = _base$sincos6[0],
        cb = _base$sincos6[1]; // (37.1) p. 264


    var x = range * cl * cb;
    var y = range * (sl * cb * cε - sb * sε);
    var z = range * (sl * cb * sε + sb * cε);
    return {
      x: x,
      y: y,
      z: z
  ...
  var c = _elliptic["default"].astrometricJ2000(f, jde, earth); // eslint-disable-line no-unused-vars


  return new _base["default"].Coord(c.ra, c.dec);
}
